...
template <
    ...
    unsigned UserBits = 0,
    typename T = void,
    typename KeyT = wjh::slotmap::Key<IndexBits, VersionBits, UserBits, T>>
constexpr auto
make_key(auto index, auto version, auto user) noexcept
{
...
{
    return make_key<IndexBits, VersionBits, UserBits, T>(
        index,
        version,
        std::uint8_t(0));
}
...
TEST_CASE("Key 32-bit: basic construction and accessors")
{
    SUBCASE("construct with all parameters") {
        constexpr auto k = make_key<20, 10, 2>(100, 5, 3);
    ...
    SUBCASE("construct without user bits (defaults to 0)") {
        constexpr auto k = make_key<20, 10, 2>(100, 5);
        static_assert(k.index() == 100);
        static_assert(k.version() == 5);
    ...
    SUBCASE("zero values") {
        constexpr auto k = make_key<20, 10, 2>(0, 0, 0);
...
TEST_CASE("Key 32-bit: default constructor creates null key")
{
    SUBCASE("default constructed key is null") {
        static_assert(std::is_default_constructible_v<Key<20, 10, 2>>);
        static_assert(
            not std::is_trivially_default_constructible_v<Key<20, 10, 2>>);
        constexpr Key<20, 10, 2> k;
        static_assert(k.index() == 0);
        static_assert(k.version() == 0);
        static_assert(k.user() == 0);
    ...
    SUBCASE("default constructed equals null()") {
        constexpr Key<20, 10, 2> k{};
        constexpr auto null_key = Key<20, 10, 2>::null();
...
TEST_CASE("Key 32-bit: null() static method")
{
    SUBCASE("null() returns all-zero key") {
        constexpr auto k = Key<20, 10, 2>::null();
        static_assert(k.index() == 0);
        static_assert(k.version() == 0);
        static_assert(k.user() == 0);
        static_assert(k.is_null());
    ...
    SUBCASE("non-null key is_null() returns false") {
        constexpr auto k = make_key<20, 10, 2>(1, 0, 0);
...
TEST_CASE("Key 32-bit: with_user() creates new key")
{
    SUBCASE("with_user modifies user bits only") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 1);
    ...
    SUBCASE("with_user preserves constexpr") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 0);
...
TEST_CASE("Key 32-bit: to_underlying() returns raw bits")
{
    SUBCASE("to_underlying for simple values") {
        constexpr auto k = make_key<20, 10, 2>(1, 1, 1);
    ...
    SUBCASE("round-trip through to_underlying") {
        constexpr auto k1 = make_key<20, 10, 2>(12345, 678, 2);
...
TEST_CASE("Key 32-bit: bit packing correctness")
{
    SUBCASE("maximum values for each field") {
        ...
        constexpr auto k = make_key<20, 10, 2>(1048575, 1023, 3);
    ...
    SUBCASE("independent bit fields don't interfere") {
        constexpr auto k1 = make_key<20, 10, 2>(1048575, 0, 0);
        constexpr auto k2 = make_key<20, 10, 2>(0, 1023, 0);
        constexpr auto k3 = make_key<20, 10, 2>(0, 0, 3);
...
TEST_CASE("Key 32-bit: different bit configurations")
{
    SUBCASE("configuration 16/16/0") {
        constexpr auto k = make_key<16, 16, 0>(65535, 65535);
    ...
    SUBCASE("configuration 24/8/0") {
        constexpr auto k = make_key<24, 8, 0>(16777215, 255);
    ...
    SUBCASE("configuration 10/10/12") {
        constexpr auto k = make_key<10, 10, 12>(1023, 1023, 4095);
...
TEST_CASE("Key 32-bit: equality comparison")
{
    SUBCASE("identical keys are equal") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 3);
        constexpr auto k2 = make_key<20, 10, 2>(100, 5, 3);
        static_assert(k1 == k2);
    ...
    SUBCASE("keys with different index are not equal") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 3);
        constexpr auto k2 = make_key<20, 10, 2>(101, 5, 3);
        static_assert(k1 != k2);
    ...
    SUBCASE("keys with different version are not equal") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 3);
        constexpr auto k2 = make_key<20, 10, 2>(100, 6, 3);
        static_assert(k1 != k2);
    ...
    SUBCASE("keys with different user are not equal") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 1);
        constexpr auto k2 = make_key<20, 10, 2>(100, 5, 2);
        static_assert(k1 != k2);
...
TEST_CASE("Key 32-bit: three-way comparison")
{
    SUBCASE("less than comparison") {
        constexpr auto k1 = make_key<20, 10, 2>(100, 5, 1);
        constexpr auto k2 = make_key<20, 10, 2>(200, 5, 1);
    ...
    SUBCASE("greater than comparison") {
        constexpr auto k1 = make_key<20, 10, 2>(200, 5, 1);
        constexpr auto k2 = make_key<20, 10, 2>(100, 5, 1);
...
{
    SUBCASE("construct with all parameters") {
        constexpr auto k = make_key<40, 20, 4>(1000000, 500000, 15);
    ...
    SUBCASE("maximum values") {
        ...
        // 4 bits: max = 2^4 - 1 = 15
        constexpr auto k = make_key<40, 20, 4>(1099511627775ULL, 1048575, 15);
...
TEST_CASE("Key 64-bit: different bit configurations")
{
    SUBCASE("configuration 32/32/0") {
        constexpr auto k = make_key<32, 32, 0>(4294967295U, 4294967295U);
    ...
    SUBCASE("configuration 48/12/4") {
        constexpr auto k = make_key<48, 12, 4>(281474976710655ULL, 4095, 15);
...
TEST_CASE("Key 64-bit: null key semantics")
{
    SUBCASE("default constructed is null") {
        constexpr Key<40, 20, 4> k{};
        static_assert(k.is_null());
        static_assert(k == Key<40, 20, 4>::null());
...
{
    constexpr auto k1 = make_key<40, 20, 4>(1000000, 500000, 5);
...
{
    SUBCASE("equality") {
        constexpr auto k1 = make_key<40, 20, 4>(1000000, 500000, 15);
        constexpr auto k2 = make_key<40, 20, 4>(1000000, 500000, 15);
        static_assert(k1 == k2);
    ...
    SUBCASE("ordering") {
        constexpr auto k1 = make_key<40, 20, 4>(1000000, 500000, 15);
        constexpr auto k2 = make_key<40, 20, 4>(2000000, 500000, 15);
...
TEST_CASE("Key 128-bit: basic construction and accessors")
{
    SUBCASE("construct with reasonable values") {
        ...
        constexpr auto k =
            make_key<80, 40, 8>(1000000000000ULL, 1000000000ULL, 255);
    ...
    SUBCASE("maximum user bits value") {
        ...
        constexpr auto k = make_key<80, 40, 8>(1, 1, 255);
        static_assert(k.user() == 255);
...
TEST_CASE("Key 128-bit: different bit configurations")
{
    SUBCASE("configuration 64/64/0") {
        constexpr auto k = make_key<64, 64, 0>(
    ...
    SUBCASE("configuration 100/20/8") {
        constexpr auto k = make_key<100, 20, 8>(1ULL << 50, 1048575, 255);
...
{
    constexpr Key<80, 40, 8> k;
    static_assert(k.is_null());
    static_assert(k == Key<80, 40, 8>::null());
...
{
    constexpr Key<80, 40, 8> k1{{1000000000000ULL}, {1000000000ULL}, {100}};
...
{
    SUBCASE("equality") {
        constexpr Key<80, 40, 8> k1{{1000000000000ULL}, {1000000000ULL}, {255}};
        constexpr Key<80, 40, 8> k2{{1000000000000ULL}, {1000000000ULL}, {255}};
        static_assert(k1 == k2);
    ...
    SUBCASE("inequality") {
        constexpr Key<80, 40, 8> k1{{1000000000000ULL}, {1000000000ULL}, {255}};
        constexpr Key<80, 40, 8> k2{{2000000000000ULL}, {1000000000ULL}, {255}};
        static_assert(k1 != k2);
...
{
    SUBCASE("member hash() is constexpr") {
        constexpr Key<80, 40, 8> k{{1000000000000ULL}, {1000000000ULL}, {255}};
        constexpr auto h = k.hash();
        static_assert(h != 0);
...
TEST_CASE("Key: type safety with phantom type parameter")
{
    SUBCASE("different phantom types are different types") {
        using K1 = Key<20, 10, 2, int>;
        using K2 = Key<20, 10, 2, unsigned>;
    ...
    SUBCASE("same phantom type and configuration are same type") {
        using K1 = Key<20, 10, 2>;
        using K2 = Key<20, 10, 2>;
...
TEST_CASE("Key: edge cases with zero user bits")
{
    ...
    SUBCASE("with_user on zero user bits key") {
        constexpr auto k1 = make_key<24, 8, 0>(16777215, 255);
...
TEST_CASE("Key: edge cases with single-bit fields")
{
    SUBCASE("1-bit user field") {
        constexpr auto k = make_key<30, 1, 1>(1073741823, 1, 1);
...
TEST_CASE("Key: to_underlying round-trip preserves all bits")
{
    SUBCASE("32-bit round-trip conceptual test") {
        ...
        constexpr auto k2 = make_key<20, 10, 2>(12345, 678, 2);
...
TEST_CASE("Key: comprehensive comparison semantics")
{
    SUBCASE("comparison is lexicographic on underlying bits") {
        ...
        auto k1 = make_key<20, 10, 2>(100, 0, 0);
        auto k2 = make_key<20, 10, 2>(100, 0, 1);
    ...
    SUBCASE("null keys are equal") {
        constexpr auto null1 = Key<20, 10, 2>::null();
        constexpr auto null2 = Key<20, 10, 2>{};
...
TEST_CASE("Key: hash quality basic check")
{
    SUBCASE("consecutive indices produce different hashes") {
        auto k1 = make_key<20, 10, 2>(100, 5, 1);
        auto k2 = make_key<20, 10, 2>(101, 5, 1);
        auto k3 = make_key<20, 10, 2>(102, 5, 1);
    ...
    SUBCASE("null key has consistent hash") {
        auto null1 = Key<20, 10, 2>::null();
        auto null2 = Key<20, 10, 2>::null();
...
TEST_CASE("Key: value semantics comprehensive test")
{
    SUBCASE("copying and assignment work as expected") {
        auto k1 = make_key<20, 10, 2>(100, 5, 3);
        ...
        auto k3 = make_key<20, 10, 2>(0, 0, 0);
